...
-- We track dropdown selections ourselves because Rayfield's CurrentValue is unreliable
local Selections = {
	AutoStartType = "Missions",
	AutoStartMap = MissionMaps[1],
	AutoStartObjective = MapObjectives[MissionMaps[1]][1],
	AutoStartDifficulty = DifficultyOptions[1],
	AutoStartModifiers = {}
}
...
local function TriggerGameStart()
	local success, err = pcall(function()
		local gameType   = Selections.AutoStartType or "Missions"
		local map        = Selections.AutoStartMap
		local objective  = Selections.AutoStartObjective
		local difficulty = Selections.AutoStartDifficulty
		local modifiers  = Selections.AutoStartModifiers or {}
...
Options.AutoStartType = Tabs.Main:CreateDropdown({
	...
	Callback = function(Value)
		...
		if selected == "Missions" then
			...
			Selections.AutoStartObjective = (MapObjectives[MissionMaps[1]] or {"Skirmish"})[1]
			...
			Selections.AutoStartMap = RaidMaps[1]
			...
			Selections.AutoStartObjective = (MapObjectives[RaidMaps[1]] or {"Skirmish"})[1]
...
Options.AutoStartMap = Tabs.Main:CreateDropdown({
	...
	Callback = function(Value)
		local selected = Value[1]
		Selections.AutoStartMap = selected
		...
		Selections.AutoStartObjective = objectives[1]
	end
})
...
Options.AutoStartObjective = Tabs.Main:CreateDropdown({
	...
	Callback = function(Value)
		Selections.AutoStartObjective = Value[1]
	end
})
...
Options.AutoStartDifficulty = Tabs.Main:CreateDropdown({
	...
	Callback = function(Value)
		Selections.AutoStartDifficulty = Value[1]
	end
})
...
Options.AutoStartModifiers = Tabs.Main:CreateDropdown({
	...
	Callback = function(Value)
		Selections.AutoStartModifiers = Value
	end
})
...
Options.MaxRewardModifier = Tabs.Main:CreateToggle({
	...
	Callback = function(Value)
		if Value then
			...
			Options.AutoStartModifiers:Set(mods)
			Selections.AutoStartModifiers = mods
		else
			Options.AutoStartModifiers:Set({})
			Selections.AutoStartModifiers = {}
		end
	end
})
